// File: app/(auth)/auth/forgot-password/page.tsx
...
import { forgotPasswordAction } from "@/action/auth/password-action";
...
export default function ForgotPasswordPage() {
	...
	const onSubmit = async (data: ForgotPasswordFormData) => {
		setIsLoading(true);
		try {
			// Call the server action with the user's email
			...
			if (result && result.status_code === 200) {
				...
				setIsSuccess(true);
				toast.success(t("auth.resetLinkSentTitle"), {
					description: result.message,
				});
			} else {
				toast.error(t("auth.errorSendingLinkTitle"), {
					description: result.message || t("auth.errorSendingLinkDescription"),
				});
			}
		} catch (error) {
			toast.error(t("auth.errorSendingLinkTitle"), {
	...
	const handleResendEmail = async () => {
		setIsLoading(true);
		try {
			// Re-use the same action to resend the email
			const result = await forgotPasswordAction({ email: submittedEmail });
